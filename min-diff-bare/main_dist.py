...
def main():
    ...
    # change from data-dir to data_dir
    ...
    # change local_rank to local-rank
    ...
    args.device = "cuda:{}".format(os.environ['LOCAL_RANK'])
    ...
    torch.manual_seed(args.seed + int(os.environ['LOCAL_RANK']))
    np.random.seed(args.seed + int(os.environ['LOCAL_RANK']))
    if int(os.environ['LOCAL_RANK']) == 0:
        print(args)
