...
IMG_DIR="/projects01/VICTRE/yubi.mamiya/minimal-diffusion/cond_data_large/"

# replace nproc_per_node w $NP
time python -m torch.distributed.launch --nproc_per_node=1 --master_port 55200 main_dist.py \
	--arch UNet --dataset skin_two --epochs 100 \
	--batch-size 256 --sampling-steps 50 --class-cond \
	--data_dir $IMG_DIR
